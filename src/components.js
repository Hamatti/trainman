...
Crafty.c('bartender', {
    init: function() {
        this.requires('Actor, spr_bartender, SpriteAnimation').animate('bartender', 0, 0, 1);
...
Crafty.c('PlayerCharacter', {
    init: function() {
        this.requires('Actor, Fourway, Collision, spr_trainplayer, SpriteAnimation, Keyboard').fourway(3).stopOnSolids().bind('KeyDown', function() {
        ...
        .animate('PlayerMovingUp', 0, 3, 2).animate('PlayerMovingRight', 0, 1, 2).animate('PlayerMovingDown', 0, 0, 2).animate('PlayerMovingLeft', 0, 2, 2);
...
function _interact(player) {
    Crafty.trigger('Interactable', {
        x: Math.round(player.at().x),
        y: Math.round(player.at().y),
        player: player
    });
}
...
function interact(player) {
    ...
    setTimeout(function() {
        document.getElementById('dialog').innerHTML = '<p>' + has_ticket + '</p>';
    }, 1000);
    calculatePoints();
}
