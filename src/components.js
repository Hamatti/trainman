...
Crafty.c( 'PlayerCharacter', {
    init: function () {
        this.requires( 'Actor, Fourway, Collision, spr_trainplayer, SpriteAnimation, Keyboard' )
            .fourway( 3 )
            .onHit( 'Bar_chair', this.visitBarTable )
            .stopOnSolids()
//            .onHit( 'Village', this.visitVillage )
    ...
    visitBarTable: function () {
        document.getElementById( 'dialog' ).innerHTML = '<p>Aint got time to sit for a drink boy..</p>';
    }
} );
...
function interact( player ) {
    ...
    setTimeout( function () {
        document.getElementById( 'dialog' ).innerHTML = '<p>' + ((Math.random() < 0.5) ? dialog.check_success : dialog.check_failure) + '</p>';
    }, 1000 );
}
